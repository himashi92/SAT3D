...
from utils.tumor_data_paths_val_server import class_mapping, all_datasets, img_datas
...
class Dataset_Union_ALL(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # Apply clamping for CT images
        # if '_ct/' in self.image_paths[index]:
        #     if 'Liver' in self.image_paths[index] or 'Lung' in self.image_paths[index] or 'Hepatic' in self.image_paths[index] or 'Pancreas' in self.image_paths[index] or 'Colon' in self.image_paths[index]:
        #         subject = tio.Clamp(-1000, 1000)(subject)
        #     else:
        #         subject = tio.Clamp(-1000, 1000)(subject)
        #         resampler = tio.Resample(target=(1.5, 1.5, 1.5), image_interpolation="linear")
        ...
        if self.mode == "train" and self.data_type == 'Tr':
            return subject.image.data.clone().detach(), binary_label
        else:
            # print(f"sub: {subject.image.data.clone().detach().shape}, gt: {binary_label.shape}")
            return subject.image.data.clone().detach(), binary_label, self.image_paths[index]
